defmodule Hive.PostReviewTest do
  use ExUnit.Case, async: false

  alias Hive.PostReview
  alias Hive.Store
  alias Hive.Test.StoreHelper

  setup do
    data_dir = Path.join(System.tmp_dir!(), "hive_test_post_review_#{:rand.uniform(1_000_000)}")
  ...
  describe "Reputation.apply_regression_penalty/1" do
    test "marks jobs with regression_detected" do
      ...
      {:ok, fresh_job} = Hive.Jobs.get(job.id)
      refute Map.get(fresh_job, :regression_detected, false)

      # Apply penalty
      Hive.Reputation.apply_regression_penalty(quest.id)
