defmodule Hive.Runtime.CrossModelAuditTest do
  use ExUnit.Case, async: true

  alias Hive.Runtime.CrossModelAudit
...
defmodule Hive.Runtime.CrossModelAudit.EnabledTest do
  use ExUnit.Case, async: false

  alias Hive.Runtime.CrossModelAudit

  setup do
    data_dir = Path.join(System.tmp_dir!(), "hive_test_audit_#{:rand.uniform(1_000_000)}")
    File.mkdir_p!(data_dir)
    Hive.Test.StoreHelper.restart_store!(data_dir)
  ...
  describe "enabled?/1" do
    ...
    test "returns false when cross_model_audit not set" do
      Hive.Store.insert(:combs, %{id: "cmb_test", path: "/tmp", name: "test"})
      refute CrossModelAudit.enabled?("cmb_test")
    end

    test "returns true when cross_model_audit is true" do
      Hive.Store.insert(:combs, %{id: "cmb_audit", path: "/tmp", name: "audit", cross_model_audit: true})
