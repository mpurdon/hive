defmodule Mix.Tasks.GiTF.Test.E2e do
  @moduledoc """
  ...
  E2E tests live in `test/e2e/` and use `GiTF.TestDriver.Scenario` for
  ...
  ## Options
    ...
    * `--remote` — connect to a running GiTF instance before running tests
  ...
  @shortdoc "Run GiTF E2E integration tests"
  ...
  def run(args) do
    ...
    if opts[:json] do
      System.put_env("GITF_E2E_JSON_REPORT", "true")
    end

    if timeout = opts[:timeout] do
      System.put_env("GITF_E2E_TIMEOUT", to_string(timeout))
    end
  ...
  defp connect_remote(node_string) do
    ...
    local_name = :"gitf_e2e_#{:erlang.unique_integer([:positive])}@127.0.0.1"

    case Node.start(local_name) do
      {:ok, _} ->
        Node.set_cookie(:gitf_test)
