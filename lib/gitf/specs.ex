defmodule GiTF.Specs do
  ...
  Spec files are markdown documents stored at `.gitf/quests/{quest_id}/{phase}.md`.
  ...
  def quest_dir(quest_id) do
    case GiTF.gitf_dir() do
      {:ok, root} -> Path.join([root, ".gitf", "quests", quest_id])
      {:error, _} -> Path.join([".gitf", "quests", quest_id])
    end
  end
end
