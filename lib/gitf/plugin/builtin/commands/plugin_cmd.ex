defmodule GiTF.Plugin.Builtin.Commands.PluginCmd do
  ...
  defp do_load(path, ctx) do
    if String.ends_with?(path, ".ex") do
      case GiTF.Plugin.Manager.load_plugin_file(path) do
      ...
      case GiTF.Plugin.Manager.load_plugin(module) do
  ...
  defp do_unload(type_name, ctx) do
    case String.split(type_name, ":", parts: 2) do
      [type_str, name_str] ->
        ...
        case GiTF.Plugin.Manager.unload_plugin(type, name_str) do
