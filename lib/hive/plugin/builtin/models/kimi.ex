defmodule Hive.Plugin.Builtin.Models.Kimi do
  @moduledoc """
  Built-in Kimi model plugin. Wraps `Hive.Runtime.Kimi` behind the
  `Hive.Plugin.Model` behaviour.
  ...
  def spawn_interactive(cwd, opts \\ []) do
    Hive.Runtime.Kimi.spawn_interactive(cwd, opts)
  end

  @impl true
  def spawn_headless(prompt, cwd, opts \\ []) do
    Hive.Runtime.Kimi.spawn_headless(cwd, prompt, opts)
  end
  ...
  def find_executable do
    Hive.Runtime.Kimi.find_executable()
  end
  ...
  def detached_command(prompt, _opts) do
    case Hive.Runtime.Kimi.find_executable() do
      {:ok, kimi_path} ->
