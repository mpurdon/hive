defmodule Hive.Plugin.Builtin.Models.Copilot do
  @moduledoc """
  Built-in GitHub Copilot model plugin. Wraps `Hive.Runtime.Copilot`
  behind the `Hive.Plugin.Model` behaviour.
  ...
  def spawn_interactive(cwd, opts \\ []) do
    Hive.Runtime.Copilot.spawn_interactive(cwd, opts)
  end

  @impl true
  def spawn_headless(prompt, cwd, opts \\ []) do
    Hive.Runtime.Copilot.spawn_headless(cwd, prompt, opts)
  end
  ...
  def find_executable do
    Hive.Runtime.Copilot.find_executable()
  end

  @impl true
  def workspace_setup(_bee_or_queen, _hive_root), do: nil
  ...
  def detached_command(prompt, _opts) do
    case Hive.Runtime.Copilot.find_executable() do
      {:ok, copilot_path} ->
