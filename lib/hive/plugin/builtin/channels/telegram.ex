defmodule Hive.Plugin.Builtin.Channels.Telegram do
  ...
  def handle_info({:shutdown, :initiated}, state) do
    do_send(state.token, state.chat_id, "Hive shutting down.")
    {:noreply, state}
  end
  ...
  defp handle_command(text, state) do
    ...
    case Hive.Plugin.Registry.lookup(:command, cmd) do
      {:ok, module} ->
  ...
  defp format_notification(event, payload) do
    case event do
      ...
      _ -> "Hive event: #{event} #{inspect(payload)}"
    end
  end
  ...
  defp flush_batch(token, chat_id, batch) do
    ...
    text = "Hive Digest (#{length(events)} events):\n\n" <> Enum.join(lines, "\n")
  ...
  defp attach_telemetry(config) do
    ...
    telemetry_events =
      Enum.map(events_config, fn event_name ->
        case event_name do
          "job_complete" -> [:hive, :job, :completed]
          "job_failed" -> [:hive, :bee, :failed]
          "quest_completed" -> [:hive, :quest, :completed]
          "bee_crashed" -> [:hive, :bee, :failed]
          _ -> nil
        end
      end)
