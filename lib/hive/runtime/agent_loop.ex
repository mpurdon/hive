defmodule Hive.Runtime.AgentLoop do
  @moduledoc """
  ...
  alias Hive.Runtime.{LLMClient, ToolBox, ModelResolver, CacheControl}
  ...
  defp prepare_context_and_cache(system_prompt, prompt, model) do
    if is_gemini?(model) and CacheControl.should_cache?(system_prompt) do
      case Hive.Runtime.GeminiCacheManager.get_or_create(system_prompt, model) do
        {:ok, cache_name} ->
